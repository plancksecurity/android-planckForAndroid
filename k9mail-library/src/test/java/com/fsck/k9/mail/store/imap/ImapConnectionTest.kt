...
private val XOAUTH_STRING_RETRY =
    "user=$USERNAME\u0001auth=Bearer $XOAUTH_TOKEN_2\u0001\u0001".base64()
private val OAUTHBEARER_STRING =
    "n,a=$USERNAME,\u0001auth=Bearer $XOAUTH_TOKEN\u0001\u0001".base64()
...
class ImapConnectionTest {
    ...
    fun `open() AUTH PLAIN failure and disconnect should throw`() {
        ...
        try {
            ...
            assertThat(e).hasMessageThat()
                .contains("Maximum number of connections from user+IP exceeded")
        }
    ...
    fun `open() AUTH CRAM-MD5`() {
        ...
        val imapConnection =
            startServerAndCreateImapConnection(server, authType = AuthType.CRAM_MD5)
    ...
    fun `open() AUTH CRAM-MD5 without AUTH CRAM-MD5 capability should throw`() {
        ...
        try {
            ...
            assertThat(e).hasMessageThat()
                .isEqualTo("Server doesn't support encrypted passwords using CRAM-MD5.")
        }
    ...
    fun `open() AUTH EXTERNAL`() {
        ...
        val imapConnection =
            startServerAndCreateImapConnection(server, authType = AuthType.EXTERNAL)
    ...
    fun `open() with connection error should throw`() {
        ...
        settings.port = 143
        val imapConnection =
            createImapConnection(settings, socketFactory, connectivityManager, oAuth2TokenProvider)
    ...
    @Test
    @Ignore("new test from K-9, still failing")
    ...
    private fun startServerAndCreateImapConnection(server: MockImapServer): ImapConnection {
        server.start()
    ...
    private fun createImapConnection(
        settings: ImapSettings, socketFactory: TrustedSocketFactory,
    ...
    private fun simpleOpenWithCapabilities(
        server: MockImapServer,
        postAuthCapabilities: String
    ): ImapConnection {
