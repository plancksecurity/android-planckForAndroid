...
class SmtpTransportTest {
    ...
    fun `open() with XOAUTH2 extension should throw on failure to fetch token`() {
        ...
        doThrow(AuthenticationFailedException("Failed to fetch token"))
    ...
    @Test
    @Ignore("need to update MockSmtpServer")
    ...
    @Test
    @Ignore("pipelining not yet implemented")
    ...
    private fun startServerAndCreateSmtpTransport(
        ...
        val uri = SmtpTransport.createUri(serverSettings)
        val storeConfig: StoreConfig = createStoreConfigWithTransportUri(uri)
        return SmtpTransport(storeConfig, socketFactory, oAuth2TokenProvider)
    }

    private fun createStoreConfigWithTransportUri(value: String): StoreConfig {
        val storeConfig = Mockito.mock(StoreConfig::class.java)
        Mockito.`when`(storeConfig.transportUri).thenReturn(value)
        return storeConfig
    }
