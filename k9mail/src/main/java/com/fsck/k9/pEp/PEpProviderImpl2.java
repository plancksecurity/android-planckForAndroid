 ...
 */
public class PEpProviderImpl2 implements PEpProvider {
    ...
    @Inject
    public PEpProviderImpl2(ThreadExecutor threadExecutor, PostExecutionThread postExecutionThread, Context context) {
    ...
    public synchronized Rating getPrivacyState(Address from, List<Address> toAddresses, List<Address> ccAddresses, List<Address> bccAddresses) {
        createEngineInstanceIfNeeded();
        return getPrivacyState(this.engine, from, toAddresses, ccAddresses, bccAddresses);
    }
    ...
    public void getPrivacyState(Address from, List<Address> toAddresses, List<Address> ccAddresses, List<Address> bccAddresses, ResultCallback<Rating> callback) {
        threadExecutor.execute(() -> {
            ...
            try {
                ...
                notifyLoaded(getPrivacyState(engine, from, toAddresses,ccAddresses, bccAddresses), callback);
            } catch (Throwable e) {
    ...
    @NonNull
    private Rating getPrivacyState(Engine engine, Address from, List<Address> toAddresses,
                                   List<Address> ccAddresses, List<Address> bccAddresses) {
    ...
    public synchronized DecryptResult decryptMessage(MimeMessage source) {
        return decrypt(this.engine, source);
    }
    ...
    public void decryptMessage(MimeMessage source, ResultCallback<DecryptResult> callback) {
        threadExecutor.execute(() -> {
            try {
                notifyLoaded(decrypt(getNewEngineSession(), source), callback);
            } catch (pEpException e) {
                notifyError(e, callback);
            }
        });
    ...
    @NonNull
    private DecryptResult decrypt(Engine engine, MimeMessage source) {
        Message srcMsg = null;
