...
import com.fsck.k9.activity.setup.AccountSetupBasics.AccountSetupSettingsCheckerFragment
...
import com.fsck.k9.mail.filter.Hex.encodeHex
...
class AccountSetupBasicsFragment : PEpFragment() {
    private lateinit var providersXmlDiscovery: ProvidersXmlDiscovery
    ...
    private var errorDialogTitle = 0
    private var errorDialogMessage: String? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        providersXmlDiscovery = ProvidersXmlDiscovery(ProvidersXmlProvider(requireContext()), createOAuthConfigurationProvider()) // TODO: fix injection
    }
    ...
    private fun saveErrorDialogState(outState: Bundle) {
    ...
    override fun onViewStateRestored(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState != null) {
            ...
            if (accountUuid != null) {
                account = Preferences.getPreferences(activity).getAccountAllowingIncomplete(accountUuid)
            }
    ...
    private fun createAccount(): Account {
        return Preferences.getPreferences(requireActivity()).newAccount()
    }

    private fun getOwnerName(): String {
        return Preferences.getPreferences(requireActivity()).defaultAccount?.name ?: ""
    }
    ...
    private fun resetView(feedback: String) {
        ...
        //enableViewGroup(true, rootView as ViewGroup?)
    }
    ...
    private fun onManualSetup() {
        ...
        val email = emailView.text?.toString() ?: error("Email missing")
        if (!emailValidator.isValidAddressOnly(email)) {
            return
        }
...
//    }
    ...
    private fun showErrorDialog(stringResource: Int, message: String?) {
        errorDialogTitle = stringResource
        errorDialogMessage = message
        errorDialog = AlertDialog.Builder(requireActivity())
            .setTitle(resources.getString(stringResource))
            .setMessage(message)
            .show()
    }
    ...
    companion object {
        ...
        private const val ERROR_DIALOG_SHOWING_KEY = "errorDialogShowing"
        private const val ERROR_DIALOG_TITLE = "errorDialogTitle"
        private const val ERROR_DIALOG_MESSAGE = "errorDialogMessage"
