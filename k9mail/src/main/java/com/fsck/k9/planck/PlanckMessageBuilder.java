...
class PlanckMessageBuilder {
    ...
    protected byte[] extractBodyContent(Body body) throws MessagingException, IOException {
        return PlanckUtils.extractBodyContent(body, true);
    }
    ...
    * ********************************************************************************************/
    ...
    private void addHeaders(Message m, Context context) {
        ...
        if (mm.getFrom()[0].getAddress() != null) {
            m.setFrom(PlanckUtils.createIdentity(mm.getFrom()[0], context));
        }
        m.setTo(PlanckUtils.createIdentities(Arrays.asList(mm.getRecipients(com.fsck.k9.mail.Message.RecipientType.TO)), context));
        m.setCc(PlanckUtils.createIdentities(Arrays.asList(mm.getRecipients(com.fsck.k9.mail.Message.RecipientType.CC)), context));
        m.setBcc(PlanckUtils.createIdentities(Arrays.asList(mm.getRecipients(com.fsck.k9.mail.Message.RecipientType.BCC)), context));
        m.setId(mm.getMessageId());
        m.setInReplyTo(createMessageReferences(mm.getHeader("In-Reply-To")));
        m.setSent(mm.getSentDate());
        Vector<Identity> identities = PlanckUtils.createIdentities(Arrays.asList(mm.getReplyTo()), context);
