...
class KeyImportPresenter @Inject constructor(private val preferences: Preferences) {
    ...
    fun initialize(view: KeyImportView, accountUuid: String) {
        ...
        email = preferences.getAccount(accountUuid).email
        val address = Address(email)
        accountIdentity = PEpUtils.createIdentity(address, context)
        ...
        return runBlocking {
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    if (id == null || !pEp.canEncrypt(email)) {
        ...
        try {
            ...
            try {
                ...
                if (importedIdentities.isEmpty()) { // This means that the file contains a key, but not a proper private key which we need.
                    ...
                    result = importedIdentities.firstOrNull { identity -> identity.address == email }
                    result?.let {
