...
@Config(sdk = [30])
class ExportPlanckSupportDataTest: RobolectricTest() {
    @get:Rule
    val coroutinesTestRule = CoroutineTestRule()
    private val context: Context = mockk()
    private val systemFileLocator: PlanckSystemFileLocator = mockk()
    private val exportPlanckSupportData = ExportPlanckSupportData(context, systemFileLocator)
    ...
    fun `export() copies files from origin to target directory using MediaStoreUtils`() {
        runBlocking {
            ...
            val result = exportPlanckSupportData(destinationBaseFolder, destinationSubFolder)
    ...
    fun `export() returns false if there is not enough free space in device`() {
        runBlocking {
            ...
            val result = exportPlanckSupportData(baseFolderSpy, destinationSubFolder)
